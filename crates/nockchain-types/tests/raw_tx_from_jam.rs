...
use nockchain_types::{BlockHeight, Nicks, RawTx};
use noun_serde::{NounDecode, NounEncode};

#[test]
fn decode_raw_tx_from_jam() -> Result<(), Box<dyn std::error::Error>> {
    const RAW_TX_JAM: &[u8] = include_bytes!("../jams/raw-tx.jam");

    let mut slab: NounSlab = NounSlab::new();
    let noun = slab.cue_into(Bytes::from_static(RAW_TX_JAM))?;

    let raw_tx = RawTx::from_noun(&noun)?;

    // basic structural checks
    assert_eq!(raw_tx.inputs.0.len(), 10, "expected ten named inputs");
    assert_eq!(raw_tx.total_fees, Nicks(0));
    assert_eq!(
        raw_tx.timelock_range.min,
        Some(BlockHeight(Belt(10))),
    ...
    let encoded = RawTx::to_noun(&raw_tx, &mut encode_slab);
    let round_trip = RawTx::from_noun(&encoded)?;

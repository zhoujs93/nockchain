...
use nockchain_types::tx_engine::note::{
    Balance, BalanceUpdate, BlockHeight, BlockHeightDelta, Nicks, Version,
};
use noun_serde::{NounDecode, NounEncode};

#[test]
fn decode_balance_from_peeks_and_snapshots() -> Result<(), Box<dyn std::error::Error>> {
    const EARLY_BALANCE_JAM: &[u8] = include_bytes!("../jams/early-balance.jam");

    let mut slab: NounSlab = NounSlab::new();
    let noun = slab.cue_into(Bytes::from_static(EARLY_BALANCE_JAM))?;

    let double_option: Option<Option<BalanceUpdate>> =
        Option::<Option<BalanceUpdate>>::from_noun(&noun)?;
    assert!(
    ...
    assert_eq!(first_note.head.version, Version::V0);
    assert_eq!(first_note.head.origin_page, BlockHeight(Belt(69)));
    ...
    assert_eq!(intent.relative.min, Some(BlockHeightDelta(Belt(4383))));
    ...
    assert_eq!(first_note.tail.assets, Nicks(2576980378));

    let mut balance_slab: NounSlab = NounSlab::new();
    let notes_noun = Balance::to_noun(notes, &mut balance_slab);
    let notes_roundtrip = Balance::from_noun(&notes_noun)?;
    ...
    let option_noun = Option::<Option<BalanceUpdate>>::to_noun(&encoded_option, &mut option_slab);
    let option_roundtrip = Option::<Option<BalanceUpdate>>::from_noun(&option_noun)?;

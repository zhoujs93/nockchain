...
use nockchain_types::tx_engine::v1::{
    BalanceUpdate, Hax, HaxPreimage as V1HaxPreimage, LockMerkleProof, LockPrimitive,
    LockTim, MerkleProof, Note, NoteData, NoteDataEntry, NoteV1, Pkh, PkhSignature,
    PkhSignatureEntry, RawTx as V1RawTx, Seed as V1Seed, Spend as V1Spend, Spend0, Spend1,
    SpendCondition, Witness as V1Witness,
};
...
impl TryFrom<PbNote> for Note {
    ...
    fn try_from(note: PbNote) -> Result<Self, Self::Error> {
        match note.note_version.ok_or(ConversionError::Invalid("missing note_version"))? {
            ...
            note::NoteVersion::V1(v1) => Ok(Note::V1(NoteV1 {
                ...
                note_data: NoteData::try_from(v1.note_data.ok_or(ConversionError::Invalid("missing note_data"))?)?,
                assets: v1.assets.ok_or(ConversionError::Invalid("missing assets"))?.into(),
            })),
        }
    }
}
...
impl TryFrom<PbSeed> for V1Seed {
    ...
    fn try_from(seed: PbSeed) -> Result<Self, Self::Error> {
        Ok(V1Seed {
            ...
            lock_root: v1::Hash::try_from(seed.lock_root.ok_or(ConversionError::Invalid("missing lock_root"))?)?,
            note_data: NoteData::try_from(seed.note_data.ok_or(ConversionError::Invalid("missing note_data"))?)?,
            gift: seed.gift.ok_or(ConversionError::Invalid("missing gift"))?.into(),
            parent_hash: v1::Hash::try_from(seed.parent_hash.ok_or(ConversionError::Invalid("missing parent_hash"))?)?,
        })
    }
}
...
impl TryFrom<PbPkhSignatureEntry> for PkhSignatureEntry {
    ...
    fn try_from(entry: PbPkhSignatureEntry) -> Result<Self, Self::Error> {
        Ok(PkhSignatureEntry {
            ...
            pubkey: entry.pubkey.ok_or(ConversionError::Invalid("missing pubkey"))?.try_into()?,
            signature: entry.signature.ok_or(ConversionError::Invalid("missing signature"))?.try_into()?,
        })
    }
}
...
impl TryFrom<PbHaxPreimage> for V1HaxPreimage {
    ...
    fn try_from(preimage: PbHaxPreimage) -> Result<Self, Self::Error> {
        Ok(V1HaxPreimage {
            hash: v1::Hash::try_from(preimage.hash.ok_or(ConversionError::Invalid("missing hash"))?)?,
...
impl TryFrom<PbLockPrimitive> for LockPrimitive {
    ...
    fn try_from(primitive: PbLockPrimitive) -> Result<Self, Self::Error> {
        match primitive.primitive.ok_or(ConversionError::Invalid("missing primitive"))? {
            lock_primitive::Primitive::Pkh(pkh) => {
                let hashes = pkh.hashes
                    .into_iter()
                    .map(v1::Hash::try_from)
                    .collect::<Result<Vec<_>, _>>()?;
                Ok(LockPrimitive::Pkh(Pkh { m: pkh.m, hashes }))
            }
            lock_primitive::Primitive::Tim(tim) => {
                Ok(LockPrimitive::Tim(LockTim {
                    rel: tim.rel.ok_or(ConversionError::Invalid("missing rel"))?.into(),
                    abs: tim.abs.ok_or(ConversionError::Invalid("missing abs"))?.into(),
                }))
            }
            lock_primitive::Primitive::Hax(hax) => {
                let hashes = hax.hashes
                    .into_iter()
                    .map(v1::Hash::try_from)
                    .collect::<Result<Vec<_>, _>>()?;
...
impl TryFrom<PbSpendCondition> for SpendCondition {
    ...
    fn try_from(condition: PbSpendCondition) -> Result<Self, Self::Error> {
        let primitives = condition.primitives
            .into_iter()
...
impl TryFrom<PbLockMerkleProof> for LockMerkleProof {
    ...
    fn try_from(proof: PbLockMerkleProof) -> Result<Self, Self::Error> {
        Ok(LockMerkleProof {
            spend_condition: SpendCondition::try_from(proof.spend_condition.ok_or(ConversionError::Invalid("missing spend_condition"))?)?,
            axis: proof.axis,
            proof: MerkleProof::try_from(proof.proof.ok_or(ConversionError::Invalid("missing proof"))?)?,
        })
    }
}
...
impl TryFrom<PbWitness> for V1Witness {
    ...
    fn try_from(witness: PbWitness) -> Result<Self, Self::Error> {
        Ok(V1Witness {
            lock_merkle_proof: LockMerkleProof::try_from(witness.lock_merkle_proof.ok_or(ConversionError::Invalid("missing lock_merkle_proof"))?)?,
            pkh_signature: PkhSignature::try_from(witness.pkh_signature.ok_or(ConversionError::Invalid("missing pkh_signature"))?)?,
            hax: witness.hax
                .into_iter()
...
impl TryFrom<PbLegacySpend> for Spend0 {
    ...
    fn try_from(spend: PbLegacySpend) -> Result<Self, Self::Error> {
        let seeds = spend.seeds
            .into_iter()
            .map(V1Seed::try_from)
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Spend0 {
            signature: spend.signature.ok_or(ConversionError::Invalid("missing signature"))?.try_into()?,
            seeds: nockchain_types::tx_engine::v1::Seeds(seeds),
            fee: spend.fee.ok_or(ConversionError::Invalid("missing fee"))?.into(),
        })
    }
}
...
impl TryFrom<PbWitnessSpend> for Spend1 {
    ...
    fn try_from(spend: PbWitnessSpend) -> Result<Self, Self::Error> {
        let seeds = spend.seeds
            ...
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Spend1 {
            witness: V1Witness::try_from(spend.witness.ok_or(ConversionError::Invalid("missing witness"))?)?,
            seeds: nockchain_types::tx_engine::v1::Seeds(seeds),
            fee: spend.fee.ok_or(ConversionError::Invalid("missing fee"))?.into(),
        })
    }
...
impl TryFrom<PbSpend> for V1Spend {
    ...
    fn try_from(spend: PbSpend) -> Result<Self, Self::Error> {
        match spend.spend_kind.ok_or(ConversionError::Invalid("missing spend_kind"))? {
...
impl TryFrom<PbSpendEntry> for (Name, V1Spend) {
    ...
    fn try_from(entry: PbSpendEntry) -> Result<Self, Self::Error> {
        Ok((
            ...
            V1Spend::try_from(entry.spend.ok_or(ConversionError::Invalid("missing spend"))?)?,
        ))
    }
}
